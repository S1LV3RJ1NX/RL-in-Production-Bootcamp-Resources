...
In Blog 2 we derived the Bellman equation — the recursive statement that connects a state's value to its successors. But having an equation isn't the same as having a number. How do we actually *compute* $V(s)$?
        ...
        for s in range(env.observation_space.n):
            ...
            for a in range(env.action_space.n):
                q = sum(p * (r + gamma * V[s_next] * (1 - done))
                        for p, s_next, r, done in env.P[s][a])  # Bellman backup
                q_values.append(q)
            V[s] = max(q_values)  # max over actions = optimality
...
An alternative to value iteration: alternate between *evaluating* a policy (solving the linear system from Blog 2) and *improving* it greedily. Often converges in fewer outer loops, but each inner step is more expensive.
...
def mc_prediction(env, policy, episodes=20000, gamma=GAMMA, alpha=0.02, max_steps=100):
    ...
    for _ in range(episodes):
        ...
        for s, reward in reversed(episode):
            G = reward + gamma * G          # G_t = r + γ * G_{t+1}
            V[s] = V[s] + alpha * (G - V[s])  # MC update
    return V
...
def td_prediction(env, policy, episodes=20000, gamma=GAMMA, alpha=0.05, max_steps=100):
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            V[s] = V[s] + alpha * (reward + gamma * bootstrap - V[s])  # TD(0) update
            s = s_next
            if terminated or truncated:
                break
    return V
...
All three converge to the same $V$ — here's the proof:
...
That leap — TD + Q-values + $\max_{a'}$ — is **Q-learning**, the subject of blog 4. Every modern algorithm (DQN, PPO, GRPO) traces back to this one-step TD update scaled up.
...
That's **Q-learning** — the algorithm that launched Deep RL when combined with a neural network (DQN). Blog 4 builds it from scratch.
