...
def fig_doom_survival() -> None:
    ...
    steps = [67, 90, 96.6, 98.3]
    errs = [0, 0, 11.0, 0]
    colors = [MUTED, INK, ACCENT, DIVIDER]
    edge = [INK, INK, INK, MUTED]
    pos = np.arange(len(labels))

    fig, ax = plt.subplots(figsize=(8.8, 4.8))
    ax.bar(pos, steps, width=0.58, color=colors, edgecolor=edge, lw=0.9,
           yerr=[e if e else np.nan for e in errs], ecolor=INK, capsize=5)
    for x, s, e in zip(pos, steps, errs):
        label = f"{s:.1f} ± {e:.0f}" if e else f"{s:.0f}"
        ax.text(x, s + (e + 3 if e else 3), label, ha="center", va="bottom",
                fontsize=10.5, color=INK, fontweight="bold")
    ax.axhline(188, ls="--", lw=1.3, color=INK)
    ax.text(-0.42, 192, 'World Models "solved" bar (188 steps / 750 tics)',
            fontsize=9.5, color=INK)
    ax.annotate("best episodes: 217-284", xy=(2, 118), xytext=(1.15, 152),
