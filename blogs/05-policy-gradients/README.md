...
The width $\sigma = 1.5$ is a deliberate choice, not a magic constant. It sets how fast reward decays as you leave the bullseye. Make it much smaller and the reward is essentially $0$ everywhere except the center, so the policy gets no "warmer or colder" signal and has to luck into angle $a_5$ before it can learn anything. Make it much larger and every angle scores near $1.0$, so the bullseye barely stands out and there is little worth learning. At $\sigma = 1.5$ the bell spans a couple of angles on each side: the bullseye clearly wins ($1.00$ versus $0.80$ for its neighbors), yet nearby angles still earn graded partial credit, which gives the policy a smooth slope to climb.
