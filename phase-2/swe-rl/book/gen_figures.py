...
BASE_STYLE = (
 "Hand-drawn explainer diagram in Excalidraw style, drawn with a fine black ink pen on a pure white "
 "background, like a smart friend sketching an idea on a whiteboard for a beginner. Slightly wobbly "
 "hand-drawn rounded rectangles and cards, thin strokes, and hand-lettered handwriting-style text for "
 "ALL labels and annotations (like the Virgil / hand-drawn font). Use a strict semantic color palette "
 "for the handwriting and accents: BLUE for how things move / mechanism / flow-of-control arrows, GREEN "
 "for numbers, rewards, scores and sizes (e.g. reward = 1.0, 73.3%, 89 tasks), RED for the bug / failure "
 "/ the 'before' state / warnings (a small cross, an error), PURPLE for literal code and shell commands "
 "in snippets (monospace-feel handwriting), ORANGE for the single 'look here' emphasis or key takeaway. "
 "Use a pale-GREEN fill for a passing / 'after' / success card and a pale-RED fill for a failing / "
 "'before' card. Long thin slightly-curved arrows connect boxes in reading order; dashed arrows connect "
 "a margin note to the thing it describes. Hand-drawn numbered circles (1)(2)(3) mark reading order. "
 "Terminal windows are dark rounded rectangles with a few purple monospace command lines and the "
 "computer's reply below. Put the key takeaway inside a dashed rounded box. Render ONLY what is "
 "described; never invent numbers or constants that are not in the description; draw colors as ink, do "
 "NOT write color names as labels. Flat, no shadows, no gradients, no photorealism, no typeset fonts, "
 "wide composition, generous white space, clean and legible handwriting."
)
